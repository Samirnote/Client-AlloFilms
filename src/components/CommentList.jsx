    ...
    apiHandler
      ...
      .then(() => {
        ...
        setComments((oldState) =>
          oldState.filter((element) => element._id !== commentId)
        );
        setShowTextArea({ id: null, show: !showTextArea.show });
      })
      .catch((err) => console.log(err));
    ...
    <>
      ...
      <ul>
        {comments.map((comment, index) => {
          return (
            <li key={comment._id}>
              <span>{comment.author.name}</span> :<span>{comment.content}</span>
              ...
              {showTextArea.id === comment._id &&
                ...
                currentUser && (
                  <>
                    ...
                    ></textarea>
                    <button
                      onClick={() => handleUpdateComment(comment._id, index)}
                    >
                      Save edit
                    </button>
                  </>
